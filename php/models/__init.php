<?php
    ...
    if(!defined('INDEX') || (!defined('INDEX') && defined('INDEX2'))) {
        if(defined('INDEX2'))
            define('INDEX', INDEX2);
        else
            define('INDEX', '../../');
        ...
        function has_no_db_error($str_activity = '', $source_location = '') {
            ...
            else if(mysqli_error($GLOBALS['con'])) {
                $error_tag = 'LIDS/POPS';
                $error_msg = mysqli_error($GLOBALS['con']);
            }
            if($error_tag != '') {
        ...
        class __init {
            ...
            public static $muncity_id    = 369; // Nabua, Camarines Sur
            public static $election_date = 'May 09, 2022';
             ...
             * @return array
             */
            protected static function get_item_helper($id, $avatar, $maintitle, $subtitle, $update_date) {
                return array(
                    'item_id'          => $id,
                    'item_avatar'      => $avatar,
            ...
            public static function _list($obj) {
                ...
                // get all records
                $GLOBALS['query'] = "SELECT `$primary_key` FROM `$table` WHERE 1 ";
                if($obj::$table == 'ph_barangays')
                    $GLOBALS['query'] .= " AND `MunCityID`=" . __init::$muncity_id . " ";
                $GLOBALS['query'] .= $list_order;
                ...
                if(has_no_db_error('for getting all ' . get_class($obj) . ' records')) {
                    ...
                    if($total > 0) {
                        ...
                        for($i=$start; $i<=$end; $i++) {
                            ...
                            $object_item = new $obj($records[$i][0], 'for listing ' . get_class($obj), false);
                            array_push($items, array(
                                'item'  => $object_item->get_item(),
                                'index' => $i
                            ));
                        }
                    }
             ...
             *
             */
            protected static function delete_helper($obj) {
            ...
            protected static function search_helper($Class) {
                $GLOBALS['query'] .= "LIMIT 300";
                $result = mysqli_query($GLOBALS['con'], $GLOBALS['query']);
                if(has_no_db_error('for searching ' . get_class())) {
                    $arr = array();
                    while($row = mysqli_fetch_assoc($result)) {
                        $object_item = new $Class($row[$Class::$primary_key], 'for SEARCH', false);
                        array_push($arr, $object_item->get_item());
                    }
